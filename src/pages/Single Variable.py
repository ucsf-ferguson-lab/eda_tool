...
if csv_file is not None:
    try:
        if "df" not in st.session_state:
            ...
            logger.info(
                f"CSV file '{csv_file.name}' loaded and saved in session_state."
            )
...
if "df" in st.session_state:
    ...
    try:
        if pd.api.types.is_numeric_dtype(df[column]):
            ...
            logger.info(f"Histogram plotted for numeric column '{column}'.")
        else:
            df[column].dropna().value_counts().plot.bar(ax=ax)
            logger.info(f"Bar chart plotted for categorical column '{column}'.")
        st.pyplot(fig)
        logger.debug("Plot displayed in Streamlit.")
    except Exception as e:
